...
function getCameraOrigin() {
  // เดิม: เช็คว่า port === '5000' เท่านั้นถึงจะใช้ origin ปัจจุบัน ไม่งั้น hardcode
  // ไปที่ 127.0.0.1:5000 ซึ่งใช้ไม่ได้เมื่อ deploy ผ่าน Docker/Coolify เพราะ
  // reverse proxy จะเสิร์ฟผ่าน port 80/443 (ไม่ใช่ 5000) และ 127.0.0.1:5000
  // จากเบราว์เซอร์ผู้ใช้จะหมายถึงเครื่องผู้ใช้เอง ไม่ใช่เซิร์ฟเวอร์
  // ใหม่: ใช้ origin ปัจจุบันเสมอ (relative to same host) เพราะ Flask เสิร์ฟ
  // ทั้งหน้าเว็บและ API จาก service เดียวกันอยู่แล้ว
